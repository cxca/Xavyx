...
- (void)viewDidLoad
{
    ...
    {
        UIBarButtonItem *logoutButton = [[UIBarButtonItem alloc]initWithTitle:@"Sign Out" style:UIBarButtonItemStylePlain target:self action:@selector(logout)];
...
- (void)likeButtonTappedOnCell:(id)sender {
    ...
    int i = indepath.row;
    {
        ...
        else
        {
            ...
            UIButton *tmpButton = likesArray[i];
            tmpButton.enabled = YES;
            [tmpButton setImage:[UIImage imageNamed:@"plusNormal"]  forState:UIControlStateNormal];
            ...
            //Update Like Button
            NSNumber *likedButton = [NSNumber numberWithInteger:0];//= (long)[dictionary objectForKey:@"liked"];
...
-(void)decrementLikes:(NSString*)idPhoto
{
    NSString *script = [NSString stringWithFormat:@"%@/%@/%@",kAPIHost,kAPIPath,@"app.php"];
    
    NSString *command = @"unlike";
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   command, @"command",
                                   idPhoto,@"idPhoto",
                                   nil];
...
-(void)getProfilePicture:(UITableViewCell *)cell;
{
    ...
    NSString *imageUrl = [NSString stringWithFormat:@"%@/%@/%@/%ld.jpg",kAPIHost,kAPIPath,@"profile",(long)appDelegate.udid];
     ...
     dispatch_async(dispatch_get_main_queue(), ^{
         // do stuff with image
         if ([UIImage imageWithData:responseObject] == nil){
             [imageButton setImage:[UIImage imageNamed:@"profilePicture"] forState:UIControlStateNormal];
         } else
...
-(void)PFregisterDeviceData
{
    ...
    if(appDelegate.deviceToken != nil)
    {
        ...
        NSString *urlString = kAPIPath;
        urlString = [urlString stringByAppendingString:@"/apns/PFregisterDevice.php?"];
